...
export async function listPriceChangeLogs(params?: { product_id?: string; page?: number; page_size?: number }) { const { product_id, ...query } = params ?? {}; return request<API.ResponseBase<API.PaginatedData>>(`/api/v1/products/${product_id ?? ''}/price-change-logs`, { method: 'GET', params: query }); }
