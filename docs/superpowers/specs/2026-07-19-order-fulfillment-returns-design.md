...
> **历史/已取代说明**：本文中的订单配送部分记录的是早期设计，已由 `docs/superpowers/specs/2026-07-19-order-delivery-management-design.md` 和 `docs/modules/delivery.md` 取代。正文保留作历史背景，其中直接调用 `PUT /api/v1/orders/{id}/deliver` 的契约不再有效；当前实现由订单出库创建一对一配送记录，并通过配送签收接口推进订单状态。退货单相关设计不受此说明影响。
