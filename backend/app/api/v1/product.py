...
from app.schemas.product import BrandCreate, BrandOut, BrandUpdate, CategoryCreate, CategoryOut, CategoryUpdate, CostPriceChangeRequest, MemberPriceBatchUpdate, MemberPriceItemOut, MemberPriceRequest, PriceChangeLogOut, ProductCreate, ProductOut, ProductUpdate, ProductWarningQuantityUpdate, SalePriceChangeRequest
...
async def create_brand(req: BrandCreate, current_user: WarehouseUser, db: AsyncSession = Depends(get_db)): return ResponseBase(data=await product_service.create_brand(db, req))
@router.put("/brands/{brand_id}", response_model=ResponseBase[BrandOut])
async def update_brand(brand_id: str, req: BrandUpdate, current_user: WarehouseUser, db: AsyncSession = Depends(get_db)):
...
async def create_category(req: CategoryCreate, current_user: WarehouseUser, db: AsyncSession = Depends(get_db)): return ResponseBase(data=await product_service.create_category(db, req))
@router.put("/categories/{category_id}", response_model=ResponseBase[CategoryOut])
async def update_category(category_id: str, req: CategoryUpdate, current_user: WarehouseUser, db: AsyncSession = Depends(get_db)):
...
async def create_product(req: ProductCreate, current_user: WarehouseUser, db: AsyncSession = Depends(get_db)):
    try: return ResponseBase(data=await product_service.create_product(db, req, current_user.username))
    except ValueError as error: _bad(error)
...
async def update_product(product_id: str, req: ProductUpdate, current_user: WarehouseUser, db: AsyncSession = Depends(get_db)):
...
async def update_product_warning_quantity(product_id: str, req: ProductWarningQuantityUpdate, current_user: WarehouseUser, db: AsyncSession = Depends(get_db)):
...
async def standard_price(product_id: str, req: SalePriceChangeRequest, current_user: WarehouseUser, db: AsyncSession = Depends(get_db)):
    try: return ResponseBase(data=await product_service.change_price(db, product_id, PriceType.standard_price, req, current_user.username))
    except ValueError as error: _bad(error)
@router.put("/{product_id}/cost-price", response_model=ResponseBase[ProductOut])
async def cost_price(product_id: str, req: CostPriceChangeRequest, current_user: WarehouseUser, db: AsyncSession = Depends(get_db)):
    try: return ResponseBase(data=await product_service.change_price(db, product_id, PriceType.cost_price, req, current_user.username))
    except ValueError as error: _bad(error)
@router.get("/{product_id}/member-prices", response_model=ResponseBase[list[MemberPriceItemOut]])
async def list_member_prices(product_id: str, current_user: CurrentUser, db: AsyncSession = Depends(get_db)):
...
async def batch_member_prices(product_id: str, req: MemberPriceBatchUpdate, current_user: WarehouseUser, db: AsyncSession = Depends(get_db)):
    try: return ResponseBase(data=await product_service.batch_update_member_prices(db, product_id, req, current_user.username))
    except ValueError as error: _bad(error)
@router.put("/{product_id}/member-prices/{level_id}", response_model=ResponseBase[ProductOut])
async def member_price(product_id: str, level_id: str, req: MemberPriceRequest, current_user: WarehouseUser, db: AsyncSession = Depends(get_db)):
    try: return ResponseBase(data=await product_service.change_price(db, product_id, PriceType.member_price, req, current_user.username, level_id))
    except ValueError as error: _bad(error)
